...
@(IsMandatory=true, Description="Type your input command for the switches", DisplayName="Type your input command for the switches")
string user_input_cmd;
...
def generateReport(context):
    ...
    show_version = 'show running-config'
    show_inventory = 'show inventory | xml'
    ...
    try:
        ...
        if platform.startswith('N'): cli_responses = show_and_store(report, serial_number, show_version, user_input_cmd, show_license)
        else:
...
def process_show_inventory(report, show_inventory):
    Logger.info('Processing show inventory')
    ...
    if "success" in show_inventory["status"]:
        ...
        module_rows = getxmlrows(content,'./TABLE_inv/ROW_inv')
        for row in module_rows:
           module_sn = getnodevalue(row,'./serialnum')
           slot = getnodevalue(row,'./name').strip('"')
           data = OrderedDict()
           data['Type'] = getnodevalue(row,'./desc').strip('"')
           data['Slot'] = slot[5:] if "Slot" in slot[:4] else ""
           data['Hardware Revision'] = getnodevalue(row,'./vendorid')
           data['Model Name'] = getnodevalue(row,'./productid')
